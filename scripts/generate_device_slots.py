    ...
    lines.extend(
        [
            ...
            *package_substitution_lines(device),
            ...
            include_line("web_profile", "!include device/web_profile.yaml"),
        ]
    )
...
def fr_list(count: int) -> str:
    return "[" + ", ".join(["FR(1)"] * count) + "]"


def lvgl_layout_substitutions(profile: dict) -> dict[str, str]:
    defaults = {
        "temperature_label_x": "8",
        "temperature_label_y": "8",
        "clock_label_y": "8",
        "network_status_x": "-8",
        "network_status_y": "1",
        "network_status_width": "48",
        "network_status_height": "44",
        "main_page_pad_bottom": "5",
        "main_page_pad_left": "5",
        "main_page_pad_right": "5",
    }
    overrides = {
        "esp32-p4-86": {
            "temperature_label_x": "12",
            "temperature_label_y": "17",
            "clock_label_y": "17",
            "network_status_x": "-20",
            "network_status_y": "10",
            "network_status_width": "44",
            "network_status_height": "42",
            "main_page_pad_bottom": "6",
            "main_page_pad_left": "8",
            "main_page_pad_right": "8",
        },
        "guition-esp32-p4-jc1060p470": {
            "network_status_x": "-4",
        },
        "guition-esp32-p4-jc4880p443": {
            "network_status_x": "-16",
            "network_status_width": "42",
            "network_status_height": "38",
        },
        "guition-esp32-p4-jc8012p4a1": {
            "temperature_label_y": "5",
            "clock_label_y": "5",
            "network_status_x": "-16",
            "network_status_width": "38",
            "network_status_height": "34",
        },
        "guition-esp32-s3-4848s040": {
            "main_page_pad_bottom": "4",
        },
    }
    data = {**defaults, **overrides.get(profile["slug"], {})}
    layout = profile["layout"]
    data["main_page_grid_columns"] = fr_list(int(layout["cols"]))
    data["main_page_grid_rows"] = fr_list(int(layout["rows"]))
    return data


def web_profile_file_text(profile: dict) -> str:
    data = {"device": profile["slug"]}
    data.update(web_config(profile))
    payload = json.dumps(data, separators=(",", ":"))
    substitutions = [
        "substitutions:",
        *[
            f'  {key}: "{value}"'
            for key, value in lvgl_layout_substitutions(profile).items()
        ],
        "",
    ]
    return "\n".join(
        [
            "# =============================================================================",
            "# WEB PROFILE - Device-owned firmware and web layout data",
            "# =============================================================================",
            "# Generated by scripts/generate_device_slots.py from devices/manifest.json.",
            "# Firmware uses the substitutions below; the browser reads the text sensor.",
            "# =============================================================================",
            "",
            *substitutions,
            "text_sensor:",
            "  - platform: template",
            '    name: "${entity_device_profile}"',
            "    id: device_profile",
            "    internal: true",
            ...
            "    lambda: |-",
            f"      return std::string(R\"espcontrol_profile({payload})espcontrol_profile\");",
            "",
        ]
    )
...
def main() -> int:
    ...
    for device in slot_devices():
        ...
        web_profile_path = ROOT / "devices" / slug / "device" / "web_profile.yaml"
        try:
        ...
        update(web_profile_path, web_profile_file_text(profiles[slug]), args.check, changed)
